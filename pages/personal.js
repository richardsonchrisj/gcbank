...
export default function Personal({ accounts }) {
  ...
  const userAccount = accounts.filter(
    (account) => account.email === userEmail || account.name === userName
  );
  ...
  return (
    <main>
      ...
      {!userAccount && (
        <div>
          <p>
            Please sign in or create an account that matches your sign-in name
            or email.
          </p>
        </div>
      )}
